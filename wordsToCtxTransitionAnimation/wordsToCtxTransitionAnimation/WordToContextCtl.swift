...
func testAnimatableTextViewCtl(onCtl: UIViewController, viewToAttachOn: UIView) -> wordsCtxTransitionTextViewCtl {
    return setupWordsCtxTransitionTextViewCtl(onCtl, viewToAttachOn: viewToAttachOn, fullContent: "When the user taps in an editable text view, that text view becomes the first responder and automatically asks the system to display the associated keyboard. Because the appearance of the keyboard has the potential to obscure portions of your user interface, it is up to you to make sure that does not happen by repositioning any views that might be obscured. Some system views, like table views, help you by scrolling the first responder into view automatically. If the first responder is at the bottom of the scrolling region, however, you may still need to resize or reposition the scroll view itself to ensure the first responder is visible.", toHighlight: "Some system views, like table views, help you by scrolling the first responder into view automatically. If the first responder is at the bottom of the scrolling region, however, you may still need to resize or reposition the scroll view itself to ensure the first responder is visible.")
}

// MARK: - Animatable textView Controller

func setupWordsCtxTransitionTextViewCtl(onCtl: UIViewController, viewToAttachOn: UIView, fullContent: String, toHighlight: String) -> wordsCtxTransitionTextViewCtl {
    let ctl = wordsCtxTransitionTextViewCtl()
    let oX = CGFloat(0)
    let oY = CGFloat(0)
    ctl.firstGlyphOriginFromListInCtlView = CGPointMake(oX, oY)
    let o = UITextView(frame: CGRectMake(oX, oY, UIScreen.mainScreen().bounds.width - oX * 2, UIScreen.mainScreen().bounds.height - oY))
    ...
    ctl.attriWords = NSMutableAttributedString(string: toHighlight)
    ...
    ctl.adjuster = UIScrollView(frame: ctl.view.frame)
    ctl.adjuster.contentSize = CGSizeMake(ctl.adjuster.frame.width, ctl.adjuster.frame.height * 3)
    ctl.adjuster.contentOffset = CGPointMake(0, ctl.adjuster.frame.height)
    ctl.view.addSubview(ctl.adjuster)
    
    let tap = UITapGestureRecognizer(target: ctl, action: "expandByTap")
...
//    }
    func scrollViewDidEndScrollingAnimation(scrollView: UIScrollView) {
        if scrollView.isKindOfClass(AnimatableOneLineTextView) {
            ...
            if let i = animatedLineExtraViews.indexOf(s) {
                if isExpanded {
                    if i - 1 >= 0 {
                    ...
                    if i + 1 < animatedLineExtraViews.count {
                        adjustToMatchLineWrap(true, currentMainView: animatedLineMainViews[i], mainViews: animatedLineMainViews, extraViews: animatedLineExtraViews)
                    } else {
                        isExpanded = true
                    }
                }
...
// MARK: - Control Animatable Lines
func adjustToMatchLineWrap(animated: Bool, currentMainView: AnimatableOneLineTextView, mainViews: [AnimatableOneLineTextView], extraViews: [AnimatableOneLineTextView]) {
    let v = getMainViewExtraViewsNeededToSync(currentMainView, mainViews: mainViews, extraViews: extraViews)
    adjustToMatchLineWrap(animated, mainViews: v.mainViews, extraViews: v.extraViews)
}

func getMainViewExtraViewsNeededToSync(currentMainView: AnimatableOneLineTextView, mainViews: [AnimatableOneLineTextView], extraViews: [AnimatableOneLineTextView]) -> (mainViews: [AnimatableOneLineTextView], extraViews: [AnimatableOneLineTextView]) {
    let i = mainViews.indexOf(currentMainView)!
    return (mainViews.filter{ mainViews.indexOf($0)! > i }, extraViews.filter{ extraViews.indexOf($0)! >= i })
}
...
func getFisrtGlyphOrigin(textView: UITextView) -> CGPoint {
    return boundingRectOriginForGlyphRange(textView, glyphRange: NSMakeRange(0, 1))
}
